      ...
      <div className="max-w-2xl mx-auto px-4 pt-16 md:pt-8">
        ...
        {!isPaid && (
          <div className={`mb-4 p-4 rounded-xl border flex items-start gap-3 ${
            ...
            <Link
              href="/pricing"
              className="text-xs text-[var(--vigil-gold)] hover:text-[var(--vigil-gold-light)] font-medium whitespace-nowrap flex items-center gap-1"
            >
        ...
        {/* Case status card */}
        {caseFile ? (
          <div className="mb-4 p-5 bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xs text-[var(--text-muted)] font-mono">CASE FILE</span>
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              ...
              <div className="text-center">
                <div className="text-lg font-bold text-[var(--text-primary)]">{PHASE_LABELS[caseFile.phase].split(' ')[0]}</div>
                <div className="text-xs text-[var(--text-muted)]">Status</div>
              </div>
            </div>
          </div>
        ) : (
          <div className="mb-4 p-5 bg-[var(--bg-card)] border border-[var(--border-default)] border-dashed rounded-2xl text-center">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <Link
            href="/chat"
            className="flex items-center gap-3 p-4 bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl hover:border-[var(--vigil-gold)]/20 transition-all group"
          >
            <div className="w-9 h-9 rounded-xl bg-[var(--vigil-gold)]/10 flex items-center justify-center">
        ...
        {recentEvidence.length > 0 && (
          <div className="mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/evidence" className="text-xs text-[var(--vigil-gold)] hover:text-[var(--vigil-gold-light)] flex items-center gap-1">
                View all <ChevronRight className="w-3 h-3" />
              </Link>
            </div>
            <div className="space-y-2">
              {recentEvidence.map(item => (
                <div key={item.id} className="flex items-start gap-3 p-3.5 bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-0.5">{item.date_observed}</p>
                  </div>
            ...
            {[
              ...
              <div
                ...
                className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all ${
                  module.locked
                    ? 'bg-[#1a1814] border-[var(--border-default)]/50 opacity-60'
                    : 'bg-[var(--bg-card)] border-[var(--border-default)] hover:border-[var(--vigil-gold)]/20'
                }`}
              >
                <div className="w-8 h-8 rounded-lg bg-[var(--bg-elevated)] flex items-center justify-center text-base shrink-0">
                  {module.locked ? <Shield className="w-4 h-4 text-[var(--text-muted)]" /> : <span>{module.icon}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-[var(--text-primary)]">Module {module.id} — {module.label}</div>
                  <div className="text-xs text-[var(--text-muted)]">{module.desc}</div>
                </div>
                {module.locked ? (
                  <Link href="/pricing" className="text-xs text-[var(--vigil-gold)] shrink-0">Unlock</Link>
                ) : (
                  <Link href="/chat" className="text-xs text-[var(--vigil-gold)] shrink-0 flex items-center gap-0.5">
        ...
        {!isPaid && (
          <div className="mt-4 p-5 bg-[var(--vigil-gold)]/5 border border-[var(--vigil-gold)]/20 rounded-2xl">
            ...
            <Link
              href="/pricing"
              className="flex items-center justify-center gap-2 bg-[var(--vigil-gold)] text-[var(--bg-primary)] py-2.5 px-5 rounded-xl text-sm font-semibold hover:bg-[var(--vigil-gold-light)] transition-all"
            >
              View plans
        ...
        {isPaid && (
          <div className="mt-4 flex items-center gap-2 p-3.5 bg-green-500/5 border border-green-500/15 rounded-xl">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">Full investigation access</p>
            </div>
          </div>
        )}
